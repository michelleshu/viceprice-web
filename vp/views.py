...
def get_location_that_needs_happy_hour(request):
    ...
    total_count = Location.objects.filter(data_entry_skipped=requiresPhone, neighborhood="North DC").count()
    locations = Location.objects.filter(data_entry_skipped=requiresPhone, dealDataManuallyReviewed=None, neighborhood="North DC").order_by('?')
