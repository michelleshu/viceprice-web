...
def get_location_that_needs_happy_hour(request):
    ...
    total_count = Location.objects.filter(data_entry_skipped=requiresPhone, neighborhood="East of The River").count()
    locations = Location.objects.filter(data_entry_skipped=requiresPhone, dealDataManuallyReviewed=None, neighborhood="East of The River").order_by('?')
